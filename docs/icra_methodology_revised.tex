...
\end{align}
$\mathcal L_{\mathrm{aux}}$ applies the state target to the sEMG-only branch
and preserves direct neuromuscular evidence. $\mathcal L_{\mathrm{mask}}$
predicts state labels hidden over random temporal spans, encouraging the model
to use causal context rather than isolated frames. During the same spans,
$\mathcal L_{\mathrm{recon}}$ reconstructs withheld sEMG activation and
regularizes the encoder against short signal loss.
...
The remaining Stage-I terms provide kinematic context: current position and
orientation describe the instantaneous arm configuration; screen and endpoint
losses associate the representation with the interaction goal; and future-pose
and future-IMU losses expose it to short-term motion dynamics. These are
auxiliary representation losses and are not deployed as final Stage-II outputs.
After checkpoint selection, the complete Stage-I classifier and both modality
encoders are frozen.
