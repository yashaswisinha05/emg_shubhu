#!/usr/bin/env python3
"""Stage 1/3: train the soft-routed model from random weights on one candidate."""
...
def main() -> None:
    if not _has("--config"):
        sys.argv[1:1] = ["--config", "configs/tracked_soft_routed_complete_reach.yaml"]
    if not _has("--output-dir"):
        sys.argv[1:1] = ["--output-dir", "runs/candidate_scratch_01_soft_routed"]
    base.build_experiment_loaders = candidate.build_candidate_loaders
    soft.main()
    candidate.save_candidate_calibration(
        _value("--output-dir", "runs/candidate_scratch_01_soft_routed")
    )
