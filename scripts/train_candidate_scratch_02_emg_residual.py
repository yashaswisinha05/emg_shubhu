#!/usr/bin/env python3
"""Stage 2/3: add temporal EMG residuals to the candidate scratch model."""
...
def main() -> None:
    ...
    if not _has("--config"):
        sys.argv[1:1] = ["--config", "configs/tracked_emg_residual_complete_reach.yaml"]
    if not _has("--output-dir"):
        sys.argv[1:1] = ["--output-dir", "runs/candidate_scratch_02_emg_residual"]
    base.build_experiment_loaders = candidate.build_candidate_loaders
    residual.main()
    candidate.save_candidate_calibration(
        _value("--output-dir", "runs/candidate_scratch_02_emg_residual")
    )
