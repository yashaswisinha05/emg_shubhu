...
class GridFusionIntentVAERegressor(nn.Module):
    """VAE whose latent is the destination and whose prior is the attractor.

    This is the VAE and the virtual-leader model as one thing rather than two
    separate branches. The pieces map onto each other directly:

      latent z      the touch location itself, not an arbitrary code
      posterior     q(z | all data), amortised by the fusion encoder
      prior         p(z | kinematics), from the attractor readout
                    r = x + (xddot + rho xdot) / eta at every timestep
      KL            between those two, replacing KL to N(0, I)

    Standard VAEs regularise toward a fixed isotropic Gaussian, which says
    nothing about the problem. Here the prior is the paper's statement that
    motion is generated by being pulled toward a goal, so the KL term is
    doing real work: it pulls the belief about where the reach is going
    toward what the observed acceleration implies. That is the Bayesian
    fusion the paper performs, written as a learned encoder.

    It also fixes what the earlier arbitrary 3-dim latent showed on real
    data: two of its dimensions correlated at -0.94, i.e. it was effectively
    2-dimensional because nothing constrained the third direction. A
    destination latent is 2-dimensional by construction, so there is no
    redundant direction to leave unconstrained.

    The prior's own spread does the trust-weighting for free. When the
    per-timestep destination estimates agree, sigma_prior is small and the KL
    pulls hard; when the reach has not committed yet they disagree,
    sigma_prior is large and the term barely applies. Nothing has to be
    hand-tuned for that.
    """
    ...
    def __init__(self, config: dict[str, Any]) -> None:
        ...
        self.virtual_leader = VirtualLeaderBranch(config)
        context_dim = 2 * d_model
        # The posterior mean is a residual on the fusion coordinate, so at
        # initialisation the model *is* grid_fusion and everything here has to
        # be earned - the lesson from the first virtual-leader attempt, which
        # replaced the coordinate outright and lost 69 px doing it.
        ...
        # sigma ~0.05 at init, matching the scale the virtual-leader spread
        # was measured at, so the sampling noise is comparable to the prior's
        # own width instead of swamping a prediction that already starts good.
        nn.init.constant_(self.posterior_log_variance[-1].bias, -6.0)
        # Identity at initialisation: the latent already lives in screen
        # coordinates, so the decoder only has to learn a correction to it.
        self.decoder = nn.Sequential(
            nn.Linear(2, 64), nn.GELU(), nn.Linear(64, 2)
        )
    ...
    def forward(self, batch: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        mean = outputs["fusion_prediction"] + self.posterior_mean(context)
        ...
        if self.training:
            ...
            latent = mean
        prediction = latent + self.decoder(latent)

        outputs["latent_mu"] = mean
        outputs["latent_log_variance"] = log_variance
        outputs["latent_sigma"] = torch.exp(0.5 * log_variance)
        # The attractor readout supplies the prior. Detached: the KL must
        # move the posterior toward the dynamics, never let the prior widen
        # itself to escape its own penalty. The prior stays honest because
        # loss.vl_weight trains it directly against the click.
        outputs["prior_mu"] = branch["vl_prediction"].detach()
        outputs["prior_sigma"] = branch["vl_sigma"].detach()
        outputs["direct_prediction"] = prediction
...
def build_grid_model(kind: str, config: dict[str, Any]) -> nn.Module:
    ...
    if kind == "grid_fusion_intent_vae":
        return GridFusionIntentVAERegressor(config)
