...
class GridFusionIntentVAERegressor(nn.Module):
    ...
    nothing about the problem. Here the prior states that motion is
    generated by being pulled toward a goal, so the KL term is doing real
    work: it pulls the belief about where the reach is going toward what
    the observed acceleration implies. That is a Bayesian fusion of
    dynamics and observation, written as a learned encoder.
