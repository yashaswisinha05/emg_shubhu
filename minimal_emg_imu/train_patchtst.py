#!/usr/bin/env python3
"""Train the causal PatchTST encoder ablation with the same four objectives."""
...
from minimal_emg_imu.patchtst_model import PatchTSTEMGIMUModel
...
def parse_args():
    ...
    parser.add_argument("--context-frames", type=int, default=600,
                        help="Past-only PatchTST context at 100 Hz")
    parser.add_argument("--chunk-frames", type=int, default=128,
                        help="Frame outputs produced per PatchTST call")
...
def main():
    ...
    def model_factory(**kwargs):
        if kwargs.get("modality") != "emg+imu":
            raise ValueError("the PatchTST ablation trains EMG+IMU only")
        return PatchTSTEMGIMUModel(
    ...
    model = PatchTSTEMGIMUModel(
        context_frames=args.context_frames, chunk_frames=args.chunk_frames)
    checkpoint.update({
        "format": "minimal_emg_imu_patchtst_v1",
        "model_args": model.model_args,
        "encoder_ablation": {
            "name": "huggingface_patchtst_random_init",
    ...
    results["protocol"].update({
        "architecture": "causal-huggingface-patchtst-ablation-v1",
        "encoder_initialization": "random",
        "modality": "emg+imu only",
        "heads": ["open/close", "current XYZ", "pixel XY",
                  "future XYZ 10--200 ms"],
        "losses": checkpoint["losses"],
        "causal_patchtst_context_frames": args.context_frames,
    })
    results_path.write_text(json.dumps(results, indent=2))
    print(f"PatchTST checkpoint: {checkpoint_path}")
